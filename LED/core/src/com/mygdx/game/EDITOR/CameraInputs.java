...
public class CameraInputs extends GestureDetector{
    ...
    public int enterButton = Keys.ENTER;
     ...
     public void update () {
         if (rotateRightPressed || rotateLeftPressed || forwardPressed || backwardPressed) {
             ...
             if (rotateRightPressed) camera.translate(tmpV1.set(camera.direction).crs(camera.up).nor().scl(-delta * translateUnits));
             if (rotateLeftPressed) camera.translate(tmpV1.set(camera.direction).crs(camera.up).nor().scl(delta * translateUnits));
             if (forwardPressed) {
